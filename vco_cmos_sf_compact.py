...
hand = [*handles[1:], handles[0]]
lab = [*labels[1:], labels[0]]
# Due to compactness the legend will not be repeated
# leg = ax_s.legend(handles=hand, labels=lab, bbox_to_anchor=(0.16,1.0,0.86,0.04), **legend_style) 
# ax_s.annotate(r"V\textsubscript{DD} I\textsubscript{b}:",xy=(0,1.08),xycoords="axes fraction",fontsize=8)
...
# ax_s2.legend(handles=handles[1:], labels=pdc_label2, **legend_style2_ncol2, bbox_to_anchor=(0,0,1,0.2), title="P\\textsubscript{DC}: min-max; avg",labelspacing=0.3)
...
ax_s.legend(handles=handles[1:], labels=eff_label, **legend_style2_ncol2_ul, bbox_to_anchor=(0.0,0.62,0.8,0.4), title="$\eta$: min-max; avg",labelspacing=0.3)
# ax_s.add_artist(leg)
# ax_s.annotate(r"V\textsubscript{DD} I\textsubscript{b}:",xy=(0,1.08),xycoords="axes fraction",fontsize=8)
for ext in ["png","pgf"]:
